...
namespace PHmiUnitTests.Tools.Dialogs.Project {
    ...
    public class ProjectDialogViewModelTests {
        ...
        public class Service : IProjectDialogService {
            public readonly Mock<INpgConnectionParameters> ConnectionParametersStub =
                new Mock<INpgConnectionParameters>();

            public readonly Mock<IConnectionStringHelper> ConnectionStringHelperStub =
                new Mock<IConnectionStringHelper>();
        ...
        private static readonly object[] BooleanCases = {true, false};
        ...
        public void CancelCommandDoesNotSetConnectionStringIfRememberIsTrue() {
            ...
            _service.ConnectionStringHelperStub
                .Verify(helper => helper.Set(PHmiConstants.PHmiConnectionStringName, It.IsAny<string>()),
                    Times.Never());
        }
        ...
        public void CancelCommandRemovesConnectionStringIfRememberIsFalse() {
            ...
            _viewModel.CancelCommand.Execute(null);
            _service.ConnectionStringHelperStub.Verify(
                helper => helper.Set(PHmiConstants.PHmiConnectionStringName, null), Times.Once());
        }
        ...
        public void OkCommandCanExecuteChangedRaisesWhenConnectionParameterPropertyChangedRaised() {
            var raised = false;
            _viewModel.OkCommand.CanExecuteChanged += (sender, args) => { raised = true; };
            _service.ConnectionParametersStub.Raise(p => p.PropertyChanged += null,
                new PropertyChangedEventArgs(null));
            Assert.IsTrue(raised);
        }

        [Test]
        [TestCaseSource("_okCommandCanExecuteTestCases")]
        public void OkCommandCanExecuteTest(bool paramsHasError, bool inProgress, bool canExecute) {
            _service.ConnectionParametersStub.Setup(p => p.Error)
                .Returns(paramsHasError ? "Error!" : string.Empty);
        ...
        public void OkCommandSavesConnectionStringIfRememberIsTrue() {
            ...
            _service.ConnectionStringHelperStub
                .Verify(helper => helper.Set(
                        PHmiConstants.PHmiConnectionStringName,
                        _viewModel.ConnectionParameters.ConnectionString),
                    Times.Once());
        }
