...
namespace PHmiRunner.Utils.Alarms {
    public class AlarmsRepository : IAlarmsRepository {
        ...
        public void EnsureTable(NpgsqlConnection connection) {
            ...
            NpgQuery createEndTimeIndexQuery =
                _npgQueryHelper.CreateIndex(_tableName, columns: DbStr.EndTime);
            NpgQuery createAckTimeIndexQuery =
                _npgQueryHelper.CreateIndex(_tableName, columns: DbStr.AcknowledgeTime);
            _npgHelper.ExecuteScript(connection,
                new[] {createTableQuery, createEndTimeIndexQuery, createAckTimeIndexQuery});
        }

        public AlarmSampleId[] GetActiveIds(NpgsqlConnection connection) {
            NpgQuery getActiveQuery = _npgQueryHelper.Select(
        ...
        public void Update(NpgsqlConnection connection, AlarmSampleId[] alarms, DateTime acknowledgeTime,
            ...
            NpgQuery query = _npgQueryHelper.UpdateWhere(
                _tableName,
                new And(
                    new Or(alarms.Select(a => new And(new Eq(DbStr.StartTime, a.StartTime.Ticks),
                        new Eq(DbStr.AlarmId, a.AlarmId))).Cast<IWhereOp>().ToArray()),
        ...
        public Alarm[] GetCurrentAlarms(NpgsqlConnection connection, CriteriaType criteriaType,
            AlarmSampleId criteria, int maxCount) {
        ...
        public Alarm[] GetHistoryAlarms(NpgsqlConnection connection, CriteriaType criteriaType,
            AlarmSampleId criteria, int maxCount) {
            return GetAlarms(connection, null, criteriaType, criteria, maxCount);
        }

        public bool HasActiveAlarms(NpgsqlConnection connection) {
            return GetAlarms(connection, new IsNull(DbStr.EndTime), CriteriaType.DownFromInfinity, null, 1)
                .Any();
        }

        public bool HasUnacknowledgedAlarms(NpgsqlConnection connection) {
            return GetAlarms(connection, new IsNull(DbStr.AcknowledgeTime), CriteriaType.DownFromInfinity,
                null, 1).Any();
        }

        private Alarm[] GetAlarms(
            NpgsqlConnection connection, IWhereOp alarmsWhereOp, CriteriaType criteriaType,
            AlarmSampleId criteria, int maxCount) {
            IWhereOp whereOp;
            bool asc;
            switch (criteriaType) {
                ...
                case CriteriaType.DownFrom:
                    whereOp = new Or(
                        new And(new Eq(DbStr.StartTime, criteria.StartTime.Ticks),
                            new Lt(DbStr.AlarmId, criteria.AlarmId)),
                ...
                case CriteriaType.DownFromOrEqual:
                    whereOp = new Or(
                        new And(new Eq(DbStr.StartTime, criteria.StartTime.Ticks),
                            new Le(DbStr.AlarmId, criteria.AlarmId)),
                        new Lt(DbStr.StartTime, criteria.StartTime.Ticks));
                    asc = false;
                    break;
                ...
                case CriteriaType.UpFrom:
                    whereOp = new Or(
                        new And(new Eq(DbStr.StartTime, criteria.StartTime.Ticks),
                            new Gt(DbStr.AlarmId, criteria.AlarmId)),
                ...
                case CriteriaType.UpFromOrEqual:
                    whereOp = new Or(
                        new And(new Eq(DbStr.StartTime, criteria.StartTime.Ticks),
                            new Ge(DbStr.AlarmId, criteria.AlarmId)),
                        new Gt(DbStr.StartTime, criteria.StartTime.Ticks));
                    asc = true;
                    break;
            ...
            NpgQuery query = _npgQueryHelper.Select(
                _tableName, _columns, whereOp, new[] {DbStr.StartTime, DbStr.AlarmId}, asc,
                Math.Min(maxCount, MaxAlarmsToRetrieve));
