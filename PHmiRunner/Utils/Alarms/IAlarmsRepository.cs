...
namespace PHmiRunner.Utils.Alarms {
    public interface IAlarmsRepository {
        ...
        void Update(NpgsqlConnection connection, AlarmSampleId[] alarms, DateTime acknowledgeTime,
            long? userId);

        void DeleteNotActive(NpgsqlConnection connection, DateTime oldStartTime);

        Alarm[] GetCurrentAlarms(NpgsqlConnection connection, CriteriaType criteriaType,
            AlarmSampleId criteria, int maxCount);

        Alarm[] GetHistoryAlarms(NpgsqlConnection connection, CriteriaType criteriaType,
            AlarmSampleId criteria, int maxCount);
