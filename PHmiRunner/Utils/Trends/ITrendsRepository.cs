...
namespace PHmiRunner.Utils.Trends {
    public interface ITrendsRepository : IDisposable {
        ...
        Tuple<DateTime, double?[]>[] GetPage(int[] trendTagIds, CriteriaType criteriaType, DateTime criteria,
            int maxCount);

        Tuple<DateTime, double?[]>[] GetSamples(int[] trendTagIds, DateTime startTime, DateTime? endTime,
            int rarerer);
    }
}