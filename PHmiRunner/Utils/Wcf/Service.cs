...
namespace PHmiRunner.Utils.Wcf {
    ...
    internal class Service : IService {
        ...
        private static void WriteDigValues(IIoDeviceRunTarget ioDevice, IList<int> ids, IList<bool> values) {
            for (var i = 0; i < ids.Count; i++) ioDevice.SetDigitalValue(ids[i], values[i]);
        }

        private static void WriteNumValues(IIoDeviceRunTarget ioDevice, IList<int> ids,
            IList<double> values) {
            for (var i = 0; i < ids.Count; i++) ioDevice.SetNumericValue(ids[i], values[i]);
        }

        private static bool?[] ReadDigValues(IIoDeviceRunTarget ioDevice, IList<int> ids) {
            var values = new bool?[ids.Count];
            for (var i = 0; i < ids.Count; i++) values[i] = ioDevice.GetDigitalValue(ids[i]);
            return values;
        }

        private static double?[] ReadNumValues(IIoDeviceRunTarget ioDevice, IList<int> ids) {
            var values = new double?[ids.Count];
            for (var i = 0; i < ids.Count; i++) values[i] = ioDevice.GetNumericValue(ids[i]);
            return values;
        }

        private static WcfNotification[] GetNotifications(IEnumerable<Notification> notifications) {
        ...
        private static void AcknowledgeAlarms(RemapAlarmsParameter parameter, IAlarmsRunTarget runTarget) {
            foreach (var p in parameter.AcknowledgeParameters) runTarget.Acknowledge(p.Item1, p.Item2);
        }
        ...
        private static void ReadCurrentAlarms(RemapAlarmsParameter parameter, RemapAlarmResult result,
            IAlarmsRunTarget runTarget) {
        ...
        private static void SaveItem(ILogMaintainer maintainer, RemapLogParameter parameter,
