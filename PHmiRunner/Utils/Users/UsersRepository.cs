...
namespace PHmiRunner.Utils.Users {
    public class UsersRepository : IUsersRepository {
        ...
        private readonly string[] _userColumns = {
            DbStr.Id,
            DbStr.Name,
            DbStr.Description,
            DbStr.Photo,
        ...
        public UpdateUserResult UpdateUser(NpgsqlConnection connection, User user) {
            try {
                int count = _npgHelper.ExecuteNonQuery(connection, _npgQueryHelper.UpdateWhere(
        ...
        public InsertUserResult InsertUser(NpgsqlConnection connection, User user) {
            try {
                int count = _npgHelper.ExecuteNonQuery(connection, _npgQueryHelper.Insert(
                    DbStr.Users,
                    new[] {
                        DbStr.Id,
                        DbStr.Name,
                ...
                switch (exception.Code) {
                    case "23505":
                        return exception.Detail.Contains(DbStr.Id)
                            ? InsertUserResult.IdConflict
                            : InsertUserResult.NameConflict;
