...
namespace PHmiRunner {
    ...
    public partial class MainWindow : IWindow {
        ...
        private void MainWindowLoaded(object sender, RoutedEventArgs e) {
            dgNotifications.Items.SortDescriptions.Add(new SortDescription("StartTime",
                ListSortDirection.Descending));
            _showStateForNotifyIcon = WindowState;
            UpdateConnectionSring();
            if (miHideAtStart.IsChecked) WindowState = WindowState.Minimized;
        }

        private void UpdateConnectionSring() {
            UpdateConnectionStringFromArgs();
            if (ViewModel.ConnectionParameters == null) UpdateConnectionSringFromConfiguration();
        }
        ...
        private void ViewModelPropertyChanged(object sender, PropertyChangedEventArgs e) {
            if (e.PropertyName == "ConnectionParameters") UpdateIconWithDispatcher();
        }

        private void ReporterPropertyChanged(object sender, PropertyChangedEventArgs e) {
            if (e.PropertyName == "ContainsActiveNotifications") UpdateIconWithDispatcher();
        }
        ...
        private void UpdateIcon() {
            if (ViewModel.ConnectionParameters == null) {
                ...
                if (_notifyIconWrapper != null) _notifyIconWrapper.SetGrayIcon();
            } else {
                if (ViewModel.Reporter.ContainsActiveNotifications) {
                    Icon = IconHelper.GetIcon(ImagesUries.RunitRed);
                    if (_notifyIconWrapper != null) _notifyIconWrapper.SetRedIcon();
                } else {
                    Icon = IconHelper.GetIcon(ImagesUries.Runit);
                    if (_notifyIconWrapper != null) _notifyIconWrapper.SetGreenIcon();
                }
            }
        }
