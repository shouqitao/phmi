...
namespace PHmiClient.Controls {
    public class NumericUpDown : Control {
        ...
        private static void
            OnMinimumPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
        ...
        private static void
            OnMaximumPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            var upDown = (NumericUpDown) d;
        ...
        public static readonly DependencyProperty ValueProperty =
            DependencyProperty.Register("Value", typeof(int?), typeof(NumericUpDown),
                new PropertyMetadata(OnValueCountChanged));

        private static void OnValueCountChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            var upDown = (NumericUpDown) d;
            var newValue = (int?) e.NewValue;
            if (newValue < upDown.Minimum) upDown.Value = upDown.Minimum;
            if (newValue > upDown.Maximum) upDown.Value = upDown.Maximum;
            upDown._downCommand.RaiseCanExecuteChanged();
            upDown._upCommand.RaiseCanExecuteChanged();
        }
