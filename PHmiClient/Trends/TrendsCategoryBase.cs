...
namespace PHmiClient.Trends {
    public class TrendsCategoryBase : TrendsCategoryAbstract {
        ...
        private readonly
            IList<Tuple<int, Tuple<CriteriaType, DateTime, int>, Action<Tuple<DateTime, double>[]>>>
            _pageQueries
                = new List<Tuple<int, Tuple<CriteriaType, DateTime, int>, Action<Tuple<DateTime, double>[]>>
                >();

        private readonly IList<Tuple<int, Tuple<DateTime, DateTime?, int>, Action<Tuple<DateTime, double>[]>>>
            _samplesQueries
        ...
        protected internal TrendsCategoryBase(int id, string name, long periodTicks) {
            _id = id;
            Name = name;
            Period = new TimeSpan(periodTicks);
        }
        ...
        protected override ITrendTag AddTrendTag(
            ...
            double? minValue,
            double? maxValue) {
            var tag = new TrendTag(this, id, name, descriptionGetter, formatGetter, engUnitGetter, minValue,
                maxValue);
            return tag;
        }
        ...
        internal override RemapTrendsParameter CreateRemapParameter() {
            ...
            if (!currentSamplesQueries.Any() && !pageSamplesQueries.Any()) return null;
        ...
        internal override void GetSamples(
            int trendTagId, DateTime startTime, DateTime? endTime, int rarerer,
            Action<Tuple<DateTime, double>[]> callback) {
            lock (_samplesQueries) {
                _samplesQueries.Add(
                    new Tuple<int, Tuple<DateTime, DateTime?, int>, Action<Tuple<DateTime, double>[]>>(
                        trendTagId, new Tuple<DateTime, DateTime?, int>(startTime, endTime, rarerer),
                        callback));
            }
        }

        internal override void GetPage(
            int trendTagId, CriteriaType criteriaType, DateTime criteria, int maxCount,
            Action<Tuple<DateTime, double>[]> callback) {
            lock (_pageQueries) {
                _pageQueries.Add(
                    new Tuple<int, Tuple<CriteriaType, DateTime, int>, Action<Tuple<DateTime, double>[]>>(
                        trendTagId, new Tuple<CriteriaType, DateTime, int>(criteriaType, criteria, maxCount),
                        callback));
            }
        }
    }
