...
namespace PHmiTools.Utils.Npg {
    public class NpgHelper : INpgHelper {
        public int ExecuteScript(
            string connectionString, NpgQuery[] script, bool useTransaction = true,
            Action<int> itemExecutedCallback = null) {
            using (var connection = new NpgsqlConnection(connectionString)) {
                return ExecuteScript(connection, script, useTransaction, itemExecutedCallback);
            }
        }

        public int ExecuteScript(NpgsqlConnection connection, NpgQuery[] script, bool useTransaction = true,
            Action<int> itemExecutedCallback = null) {
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            var count = 0;
            NpgsqlTransaction transaction = useTransaction ? connection.BeginTransaction() : null;
        ...
        public int CreateDatabase(INpgConnectionParameters connectionParameters) {
            return CreateDatabase(connectionParameters.ConnectionStringWithoutDatabase,
                connectionParameters.Database);
        }

        public int CreateDatabase(string connectionString) {
            string database = GetDatabase(connectionString);
            string connectionsStringWithoutDatabase = GetConnectionStringWithoutDatabase(connectionString);
        ...
        public T[] ExecuteReader<T>(string connectionString, NpgQuery query,
            Func<NpgsqlDataReader, T> convertFunc) {
            using (var connection = new NpgsqlConnection(connectionString)) {
                return ExecuteReader(connection, query, convertFunc);
            }
        }

        public T[] ExecuteReader<T>(NpgsqlConnection connection, NpgQuery query,
            Func<NpgsqlDataReader, T> convertFunc) {
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            using (var command = new NpgsqlCommand(query.Text, connection)) {
                ...
                while (reader.Read()) result.Add(convertFunc.Invoke(reader));
        ...
        public string[] GetDatabases(INpgConnectionParameters connectionParameters) {
            return GetDatabasesFromConnectionStringWithoutDatabase(connectionParameters
                .ConnectionStringWithoutDatabase);
        }

        public string[] GetDatabases(string connectionString) {
            return GetDatabasesFromConnectionStringWithoutDatabase(
                GetConnectionStringWithoutDatabase(connectionString));
        }
        ...
        private int CreateDatabase(string connectionStringWithoutDatabase, string database) {
            ...
            int count = ExecuteScript(connectionStringWithoutDatabase, script, false);
            Thread.Sleep(1000);
            return count;
        }

        private string GetConnectionStringWithoutDatabase(string connectionStringWithDatabase) {
            string database = GetDatabase(connectionStringWithDatabase);
