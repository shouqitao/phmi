...
namespace PHmiTools.Utils.Npg {
    public class NpgQueryHelper : INpgQueryHelper {
        ...
        public NpgQuery CreateTable(NpgTableInfo tableInfo) {
            ...
            foreach (NpgColumnInfo columnInfo in tableInfo.Columns)
        ...
        public NpgQuery CreateColumn(string table, NpgColumnInfo column) {
            string text = string.Format("ALTER TABLE {0} ADD COLUMN {1} {2} {3};",
                table, column.Name, column.DataType,
                column.NotNull ? "NOT NULL" : "NULL");
            return new NpgQuery(text);
        }

        public NpgQuery CreateIndex(string table, NpgIndexType indexType, bool unique,
        ...
        public string Select(
            ...
            var text = new StringBuilder();
            text.Append("SELECT ");
            if (distinct) text.Append("DISTINCT ");
            text.AppendFormat(
                "{0} FROM {1}",
                string.Join(", ", columnsToReturn.Select(Quote)),
                Quote(table));
            if (whereOp != null) text.AppendFormat(" WHERE {0}", whereOp.Build(parameters));
            ...
            if (limit != -1) text.AppendFormat(" LIMIT {0}", limit);
        ...
        public NpgQuery Select(
            ...
            string text = Select(parameters, table, columnsToReturn, whereOp, columnsOfOrder, asc, limit,
                distinct);
        ...
        public NpgQuery Insert(string table, string[] columns, IEnumerable<object[]> values) {
            ...
            string scriptRow = string.Format(
        ...
        public NpgQuery UpdateWhere(string table, IWhereOp whereOp, string[] columns, object[] values) {
            ...
            for (var i = 0; i < set.Length; i++) {
                ...
                string parameterName = SetParameterNamePrefix + column;
        ...
        public NpgQuery DeleteWhere(string table, IWhereOp whereOp) {
            var parameters = new List<NpgsqlParameter>();
            string text = string.Format("DELETE FROM {0} WHERE {1}",
                Quote(table),
                whereOp.Build(parameters));
            return new NpgQuery(text, parameters.ToArray());
        }
