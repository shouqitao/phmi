...
namespace PHmiTools.Utils.Npg.ExIm {
    public class NpgExImHelper : INpgExImHelper {
        ...
        public NpgColumnInfo[] GetColumns(string scriptRow) {
            ...
            Match match = regex.Match(scriptRow);
            while (match.Success) {
        ...
        public string[] GetPrimaryKey(string scriptRow) {
            ...
            Match match = regex.Match(scriptRow);
            if (match.Success) {
                string keysStr = match.Groups[2].ToString();
                var findKeysRegex = new Regex(@"[\w_]+");
                Match findKeysMatch = findKeysRegex.Match(keysStr);
        ...
        public NpgTableInfo GetTable(string scriptRow) {
            ...
            Match match = regex.Match(scriptRow);
            if (match.Success) {
                string tableName = match.Groups[1].ToString();
                string body = match.Groups[2].ToString();
        ...
        public NpgQuery GetSelectScriptItem(NpgTableInfo tableInfo, int maxRows,
            params KeyValuePair<string, object>[] startParameters) {
            string scriptParameters =
                startParameters.Any()
                    ? " WHERE " + string.Join(" AND ",
                          startParameters.Select(p => string.Format("\"{0}\" > :{0}", p.Key)))
                    : string.Empty;
        ...
        public NpgQuery GetInsertScriptItem(TableData tableData) {
            ...
            for (var i = 0; i < tableData.Data.Length; i++) {
                ...
                for (var j = 0; j < tableData.Columns.Length; j++) {
                    string column = tableData.Columns[j];
                    object value = data[j];
