...
namespace PHmiTools.Utils.Npg {
    public interface INpgHelper {
        int ExecuteScript(string connectionString, NpgQuery[] script, bool useTransaction = true,
            Action<int> itemExecutedCallback = null);

        int ExecuteScript(NpgsqlConnection connection, NpgQuery[] script, bool useTransaction = true,
            Action<int> itemExecutedCallback = null);
        ...
        T[] ExecuteReader<T>(NpgsqlConnection connection, NpgQuery query,
