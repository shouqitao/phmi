...
namespace PHmiConfigurator.Modules.Collection.Selectable {
    public class
        DigitalTagsViewModel : SelectableCollectionViewModel<DigTag, DigTag.DigTagMetadata, IoDevice> {
            ...
            get {
                ...
                if (CurrentSelector != null) {
                    ...
                    var numericTagsNames = Context.Get<NumTag>()
                        ...
                        .ToDictionary(n => n);
                    var names = List.Where(t => numericTagsNames.ContainsKey(t.Name)).Select(t => t.Name)
                        .ToArray();
                    if (names.Any()) {
                        if (!string.IsNullOrEmpty(error))
                            error += Environment.NewLine;
                        error += string.Format(Res.NumericTagPresentMessage,
