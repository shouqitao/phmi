...
namespace PHmiConfigurator.Modules.Collection.Selectable {
    public class
        NumericTagsViewModel : SelectableCollectionViewModel<NumTag, NumTag.NumTagMetadata, IoDevice> {
        ...
        public override string Error {
            get {
                ...
                if (CurrentSelector != null) {
                    int ioDeviceId = CurrentSelector.Id;
                    var digitalTagsNames = Context.Get<DigTag>()
                        .Where(t => t.RefIoDevice == ioDeviceId).Select(t => t.Name).Distinct()
                        .ToDictionary(n => n);
                    var names = List.Where(t => digitalTagsNames.ContainsKey(t.Name)).Select(t => t.Name)
                        .ToArray();
                    if (names.Any()) {
                        if (!string.IsNullOrEmpty(error))
                            error += Environment.NewLine;
                        error += string.Format(Res.DigitalTagPresentMessage,
                                     ReflectionHelper.GetDisplayName<DigTag>(t => t.Name))
                                 + Environment.NewLine
                                 + string.Join(", ", names) + ".";
                    }
                }
