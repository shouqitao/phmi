...
namespace PHmiIoDevice.Melsec.Implementation {
    internal abstract class FxBase : IMelsec {
        ...
        public void WriteRegisters(int address, List<byte> data) {
            ...
            var summ = 0;
            for (var i = 1; i < message.Length; i++) summ += (byte) message[i];
            string checkSumm = IncludeFirstZero(((byte) (summ & 0xFF)).ToString("X"), 2);
            message.Append(checkSumm);

            Write(Encoder.GetBytes(message.ToString()));

            var bytes = Read(1);
            if (bytes.Count == 1 && bytes[0] == 0x06) return;

            var error = "Error";
            if (bytes.Count > 0) error += " code " + bytes[0];
        ...
        public void WriteMerker(int address, bool data) {
            ...
            int dataType = 0x4000 + address;
            string str = IncludeFirstZero(dataType.ToString("X"), 4);
        ...
        private List<byte> Read(int dataType, int dataSize) {
            ...
            if (bytes == null || bytes.Count < 5) throw new Exception("Recieved message size error");

            {
                ...
                for (var i = 0; i < endIndex / 2; i++) {
                    if (i + 1 >= endIndex)
                        continue;
                    int index = i * 2 + 1;
                    string str = Encoder.GetString(new[] {bytes[index], bytes[index + 1]});
