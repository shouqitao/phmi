...
namespace PHmiIoDevice.Melsec.Implementation {
    internal class Q : IMelsec {
        ...
        public Q(string address, int port, byte pcNumber, byte networkNumber, int timeout,
            int messageEndTimeout) {
            _pcNumber = pcNumber;
        ...
        public void WriteRegisters(int address, List<byte> data) {
            ...
            int requestLength = 12 + data.Count;
            ...
            if (answer == null) throw new Exception("Message not received");
            if (answer.Count != messageLengthToReceive) throw new Exception("Recieved message size error");
            var responseCode = (int) answer[10];
            responseCode = responseCode << 8;
            responseCode = responseCode | answer[9];
            if (responseCode != 0) throw new Exception("Response code 0x" + responseCode.ToString("X"));
        }
        ...
        private List<byte> ReadBlock(int address, int length, byte code) {
            ...
            int bytesToReceive = length * 2;
            const int headerLength = 11;
            int messageLengthToReceive = bytesToReceive + headerLength;
            var answer = _tcpHelper.Read(messageLengthToReceive);
            if (answer == null) throw new Exception("Message not received");
            ...
            if (answer.Count != messageLengthToReceive) throw new Exception("Recieved message size error 1");
            ...
            for (var i = 0; i < bytesToReceive; ++i) data.Add(answer[headerLength + i]);
            return data;
        }
