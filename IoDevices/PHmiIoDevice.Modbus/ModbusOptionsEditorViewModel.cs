...
namespace PHmiIoDevice.Modbus {
    public class ModbusOptionsEditorViewModel : INotifyPropertyChanged {
        ...
        public AsciiConfig AsciiConfig {
            get { return _config as AsciiConfig; }
        }

        public AsciiViaTcpConfig AsciiViaTcpConfig {
            get { return _config as AsciiViaTcpConfig; }
        }

        public ComConfig ComConfig {
            get { return _config as ComConfig; }
        }
        ...
        public RtuConfig RtuConfig {
            get { return _config as RtuConfig; }
        }

        public RTUviaTCPConfig RtuViaTcpConfig {
            get { return _config as RTUviaTCPConfig; }
        }

        public TcpConfig TcpConfig {
            get { return _config as TcpConfig; }
        }
        ...
        private void UpdateConfigType() {
            ...
            else if (TcpConfig != null) ConfigType = ConfigType.Tcp;
        }

        private void UpdateConfig() {
            switch (ConfigType) {
                case ConfigType.Ascii:
                    if (AsciiConfig == null) Config = new AsciiConfig();
                    break;

                case ConfigType.AsciiViaTcp:
                    if (AsciiViaTcpConfig == null) Config = new AsciiViaTcpConfig();
                    break;

                case ConfigType.Rtu:
                    if (RtuConfig == null) Config = new RtuConfig();
                    break;

                case ConfigType.RtuViaTcp:
                    if (RtuViaTcpConfig == null) Config = new RTUviaTCPConfig();
                    break;

                case ConfigType.Tcp:
                    if (TcpConfig == null) Config = new TcpConfig();
                    break;
        ...
        private static void ShowDocCommandExecuted(object obj) {
            try {
                string assemblyLocation = Assembly.GetAssembly(typeof(ModbusOptionsEditorViewModel)).Location;
        ...
        protected virtual void OnPropertyChanged(
            Expression<Func<ModbusOptionsEditorViewModel, object>> getPropertyExpresstion) {
            string property = PropertyHelper.GetPropertyName(this, getPropertyExpresstion);
