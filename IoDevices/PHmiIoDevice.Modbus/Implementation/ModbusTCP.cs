...
namespace PHmiIoDevice.Modbus.Implementation {
    internal class ModbusTCP : ModbusBase {
        ...
        protected override List<byte> Read(
            ...
            if (response == null || response.Count < 9) throw new Exception("No response");

            int packLength = response.Count - 6;
            ...
            if (slaveAddress != response[6]) throw new Exception("Slave address does not match");
            ...
            if (response.Count != byteLength + 9) throw new Exception("Unexpected packet length");

            if (pdu[0] != response[7]) throw new Exception("Function code does not match");

            if (byteLength != response[8]) throw new Exception("Bytes count does not match");
            int dataBytesLength = response.Count - 9;
            if (byteLength != dataBytesLength) throw new Exception("Data bytes count does not match");

            var result = new List<byte>(byteLength);
            for (var i = 9; i < byteLength + 9; ++i) result.Add(response[i]);
        ...
        protected override bool Write(
            ...
            if (response.Count != 12) throw new Exception("Unexpected packet length");
        ...
        private static byte[] CreateAdu(byte slaveAddress, ICollection<byte> pdu) {
            int length = pdu.Count + 1;
            var lengthByteH = (byte) ((length >> 8) & 0xFF);
            var lengthByteL = (byte) (length & 0xFF);
            var adu = new List<byte>(pdu.Count + 7)
                {0x00, 0x00, 0x00, 0x00, lengthByteH, lengthByteL, slaveAddress};
            adu.AddRange(pdu);
            return adu.ToArray();
        }
