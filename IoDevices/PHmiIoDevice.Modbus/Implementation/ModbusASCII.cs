...
namespace PHmiIoDevice.Modbus.Implementation {
    internal class ModbusASCII : ModbusBase {
        ...
        protected override List<byte> Read(
            ...
            int packetLength = (byteLength + 4) * 2 + 3;
            var responseAscii = _portHelper.Read(packetLength);
            if (responseAscii == null || responseAscii.Count < 11) throw new Exception("No response");
            var response = GetBytes(responseAscii.ToArray());
            byte lrc = GetLrc(response, 0, response.Count - 1);
            if (lrc != response[response.Count - 1]) throw new Exception("LRC error");
            if (slaveAddress != response[0]) throw new Exception("Slave address does not match");
            ...
            if (responseAscii.Count != packetLength) throw new Exception("Unexpected packet length");

            if (pdu[0] != response[1]) throw new Exception("Function code does not match");

            if (byteLength != response[2]) throw new Exception("Bytes count does not match");
            int dataBytesLength = response.Count - 4;
            if (byteLength != dataBytesLength) throw new Exception("Data bytes count does not match");

            var result = new List<byte>(byteLength);
            for (var i = 3; i < byteLength + 3; ++i) result.Add(response[i]);
        ...
        protected override bool Write(
            ...
            if (responseAscii.Count != 17) throw new Exception("Unexpected packet length");
        ...
        private static byte GetLrc(IList<byte> message, int startIndex, int length) {
            var summ = 0;
            for (int i = startIndex; i < startIndex + length; ++i) summ += message[i];
            int bitReversal = (summ ^ 0xFF) & 0xFF;
            int result = (bitReversal + 1) & 0xFF;
            return (byte) result;
        }

        private static byte[] GetAscii(ICollection<byte> adu) {
            var ascii = new List<byte>(adu.Count * 2 + 3) {0x3A};
            foreach (byte b in adu) ascii.AddRange(ByteToAscii(b));
            ascii.Add(0x0D);
            ascii.Add(0x0A);
