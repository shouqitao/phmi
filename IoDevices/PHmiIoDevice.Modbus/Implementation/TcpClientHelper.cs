    ...
    internal class TcpClientHelper : IHelper {
        ...
        public List<byte> Read(int messageLength) {
            ...
            while (!stream.DataAvailable) {
                if (++tryIndex > _retries) throw new TimeoutException("Timeout when reading");
                Thread.Sleep(_delayTime);
            }
            ...
            while (stream.DataAvailable && result.Count < messageLength) {
                ...
                int length = stream.Read(message, 0, messageLength);
                for (var i = 0; i < length; ++i) result.Add(message[i]);
                Thread.Sleep(_delayTime);
            }
