    ...
    public class ModbusIoDevice : IIoDevice {
        ...
        private void GetParametersReadInfo(
            ...
            foreach (ReadParameter parameter in readParameters) {
                ...
                byte slaveAddress = _defaultAddress;
                string match1 = match.Groups[1].ToString();
                ...
                deviceAddress -= 1;
                string match2 = match.Groups[2].ToString();
                int? length;
        ...
        private static void ReadBits(
            ...
            foreach (ushort address in bitsAddresses)
                if (length == 0) {
                    readStart = address;
                    length = 1;
                    ...
                    int newLength = address - readStart + 1;
        ...
        public void WriteOnce(WriteParameter[] writeParameters) {
            ...
            foreach (var slaveWriteInfo in writeInfos) {
                byte slaveAddress = slaveWriteInfo.Key;
                SlaveWriteInfo writeInfo = slaveWriteInfo.Value;
                SortWriteInfoDevices(writeInfo);
                for (var i = 0; i < writeInfo.Coils.Count; ++i) {
                    CoilWriteInfo coil = writeInfo.Coils[i];
                    ushort address = coil.Address;
                    ...
                    for (var j = 1; j < maxLength && i + j < writeInfo.Coils.Count; ++j) {
                        CoilWriteInfo c = writeInfo.Coils[i + j];
                    ...
                    if (_mb.WriteCoils(slaveAddress, address, BoolsToBytes(bools), bools.Count))
                        for (int j = i; j < bools.Count + i; ++j)
                            result[writeInfo.Coils[j].Index] = true;
                    i += bools.Count - 1;
                }

                for (var i = 0; i < writeInfo.HoldingRegisters.Count; ++i) {
                    HoldingRegisterWriteInfo hr = writeInfo.HoldingRegisters[i];
                    ...
                    for (var j = 1; i + j < writeInfo.HoldingRegisters.Count; ++j) {
                        HoldingRegisterWriteInfo h = writeInfo.HoldingRegisters[i + j];
        ...
        private void GetParametersWriteInfo(
            ...
            int index = -1;
            foreach (WriteParameter parameter in parameters) {
                ++index;
                var regex = new Regex(DevicePattern);
                Match match = regex.Match(parameter.Address);
                if (!match.Success) throw new Exception(parameter.Address + " is not a valid device address");
